...
const HomePage = ({ setLoading }) => {
    ...
    const ProductbyCategory = (data) => {
        api.getProductbyCategory(data)
            .then(res => {
                console.log(res);
                const Prod = res.data;
                Prod.map(result => {
                    Object.assign(result, { quantity: result.productQuantity })
                })
                setProductByCategory(Prod)
            })
            .catch(err => {
                console.log(err);
    ...
    const addToWishlist = (data) => {
        ...
        api.AddwishList(post)
            .then((res) => {
                ...
                triggerNotification('success', 'Success', 'Added in Wishlist', 'x', null)
    ...
    const IncreaseQuantity = (index) => {
        const updatedCart = [...ProductByCategory];
        console.log(updatedCart[index]);
        updatedCart[index].productQuantity += 1;
        setProductByCategory(updatedCart);
    };

    const DecreaseQuantity = (index) => {
        const updatedCart = [...ProductByCategory];
        if (updatedCart[index].productQuantity > 1) {
            updatedCart[index].productQuantity -= 1;
            setProductByCategory(updatedCart);
        }
    };
                                ...
                                <ul className="nav nav-tabs best-selling-grocery" id="myTab" role="tablist">
                                    {
                                        allCategories.map(res => {
                                            return (
                                                <li className="nav-item" role="presentation" onClick={() => clickTab(res._id, res.name)} key={res._id}>
                                                    <button
                                                        className={`nav-link ${res._id == SelectedTab ? 'avtiveCategory' : ''}`}
                                                        id={`tab-${res._id}`}
                                                        data-bs-toggle="tab"
                                                        data-bs-target={`#tab-${res._id}`}
                                                        type="button"
                                                        role="tab"
                                                        aria-controls={`tab-${res._id}`}
                                                        aria-selected={res._id == SelectedTab}
                                                    >
                                                        {res.name}
                                                    </button>
                                                </li>
                    ...
                    <div className="row">
                        <div className="col-lg-12">
                            <div className="tab-content" id="myTabContent">
                                <div className="tab-pane fade show active" id={SelectedTab} role="tabpanel" aria-labelledby={SelectedTab}>
                                    <div className="row g-4">
                                        {
                                            ProductByCategory.map((product, index) => (
                                                <div className="col-xxl-2 col-xl-3 col-lg-4 col-md-4 col-sm-6 col-12" key={product._id}>
                                                    <div className="single-shopping-card-one">
                                                        <div className="image-and-action-area-wrapper">
                                                            <a href={`/productDetail?productId=${product._id}`} className="thumbnail-preview w-100">
                                                                <div className="badge">
                                                                    <span>{product.productDiscount}% <br />Off</span>
                                                                    <i className="fa-solid fa-bookmark"></i>
                                                                </div>
                                                                <img className='ProdImage' src={product.productImage} alt={product.productName} />
                                                            </a>
                                                            <div className="action-share-option">
                                                                <span className="single-action openuptip" title="Add To Wishlist" onClick={() => addToWishlist(product)}>
                                                                    <i className="fa-light fa-heart"></i>
                                                                </span>
                                                                <span className="single-action openuptip" title="Compare">
                                                                    <i className="fa-solid fa-arrows-retweet"></i>
                                                                </span>
                                                                <span className="single-action openuptip" title="Quick View" onClick={() => ProductDetail(product._id)}>
                                                                    <i className="fa-regular fa-eye"></i>
                                                                </span>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div className="body-content">
                                                            <a onClick={() => ProductDetail(product._id)}>
                                                                <h4 className="title d-flex">{product.productName}</h4>
                                                            </a>
                                                            <span className="availability d-flex">{product.quantity} {product.productUnit}</span>
                                                            <div className="price-area d-flex justify-content-flex-start">
                                                                <span className="current">₹ {product.productCurrentRate}</span>
                                                                <div className="previous">₹ {product.productOriginalRate}</div>
                                                            </div>
                                                            <div className="cart-counter-action">
                                                                <div className="quantity-edit">
                                                                    <input type="text" className="input" value={product.productQuantity} readOnly />
                                                                    <div className="button-wrapper-action">
